...
import GYGWidget from "./widgets/GYGWidget";
...
export default function Timeline({ plan }: TimelineProps) {
  if (!plan || !plan.days) {
    return <p className="text-center text-white/50 p-10">Błąd danych planu.</p>;
  }
  ...
  return (
    <>
      {plan.days.map((day, index) => {
        ...
        const destination = day.location || day.location_en;
        ...
        return (
          <div key={day.day_number}>
            ...
            >
              ...
              <div className="mb-10 w-full h-80 bg-black/40 rounded-2xl border border-white/10 overflow-hidden relative group shadow-2xl">
                <img
                  ...
                  alt={day.location}
                  loading="lazy"
                  className="w-full h-full object-cover opacity-0 transition-all duration-1000 group-hover:scale-105"
                  onLoad={(e) => {
                      ...
                      <div className="col-span-3 text-gray-200 leading-relaxed text-base md:text-lg">
                        <div
                          dangerouslySetInnerHTML={{
                            __html: parseMarkdown(act.description),
                          }}
                        />
                        {act.maps_url && (
                          <a
            ...
            {/* Widget GYG tylko pod pierwszym dniem */}
            {isFirstDay && <GYGWidget destination={destination} />}
          </div>
        );
      })}
    </>
...
// Prosta funkcja parsująca Markdown (tylko pogrubienia)
