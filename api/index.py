...
app = Flask(
    __name__, static_folder=os.path.join(ROOT_DIR, "static"), static_url_path="/static"
)
CORS(app)
...
# POPRAWIONY ENDPOINT: Zmieniono nazwę na /api/plan/ zgodnie ze script.js
...
def get_plan(plan_id):
    ...
    try:
        ...
        if response.data and len(response.data) > 0:
            # KLUCZOWA ZMIANA: Zwracamy obiekt plan_data bezpośrednio,
            # bo tam znajduje się struktura {"plan": ..., "sources": ...}
            # której oczekuje Twój frontend
            return jsonify(response.data[0]["plan_data"])
        else:
...
# ENDPOINT: Generowanie planu (bez zmian, dla porządku)
...
def handle_plan():
    try:
        ...
        return jsonify({"plan": plan, "sources": sources, "id": plan_id})
